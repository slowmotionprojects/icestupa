...
"""
Fake DNS server, adapted with minor modifications from https://github.com/Crypt0s/FakeDns
DNS queries on domains listed in dns.conf (with associated lookup type) return the specified IP, while queries on all
other domains/lookup types are forwarded to Google's DNS (8.8.8.8).
"""
        ...
        for rule in self.re_list:
            ...
            if rule[1].match(query.dominio):
                ...
                if query.type in TYPE.keys() and rule[0] == TYPE[query.type]:
                    ...
                    print("[%s] Matched DNS request - " % datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f') + query.dominio)
